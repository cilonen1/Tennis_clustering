...
def count_sum(continent, ctry_border, feat, cols, year, kmeans, cl_num):
    ...
    df_cont = pd.read_excel(r'C:\Users\cilon\OneDrive\Desktop\New-Tennis\Chal_full_by_cont.xlsx',header=0, sheet_name=continent)[st:end][cols]
    ...
    for i in range(cl_num):
        ...
        df_final['money1'] =(np.where(df_final.win == 1, 100 * df_final.bet1, (-100/(df_final.k1 - 1)* df_final.bet1).astype(int)))
        df_final['money2'] =(np.where(df_final.win == 0, 100 * df_final.bet2, (-100/(df_final.k2 - 1)* df_final.bet2).astype(int)))
...
def full_cycle(continent, year=2019):
    ...
    dx.fillna({'Rank2':1200,'Rank1':1200}, inplace=True)#берем все челленджеры чтобы из них один раз сделать кластеры
